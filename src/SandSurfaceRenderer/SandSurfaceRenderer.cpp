...
void SandSurfaceRenderer::setup(bool sdisplayGui){
    ...
    {
        heightMap.createFile(colorMapPath+"HeightColorMap2.xml");
        vegetationType = "HeightColorMap2.xml";
        saveSettings();
        settingsLoaded = true;
    }
...
void SandSurfaceRenderer::setupGui(){
	...
	int pos = find(colorMapFilesList.begin(), colorMapFilesList.end(), vegetationType) - colorMapFilesList.begin();
...
void SandSurfaceRenderer::onButtonEvent(ofxDatGuiButtonEvent e){
    if (e.target->is("Save")) {
        ...
        heightMap.loadFile(colorMapPath+vegetationType);
        populateColorList();
...
void SandSurfaceRenderer::onDropdownEvent(ofxDatGuiDropdownEvent e){
    vegetationType = e.target->getLabel();
	
	cout << "Current type of vegetation is : " << vegetationType << endl;
...
bool SandSurfaceRenderer::loadSettings(){
    ...
    vegetationType = xml.getValue<string>("colorMapFile");
...
bool SandSurfaceRenderer::saveSettings(){
    ...
    xml.setTo("SURFACERENDERERSETTINGS");
    xml.addValue("colorMapFile", vegetationType);
